...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    echo "##########################################################"
    echo "############ Create OrgTourist Identities ######################"
    echo "##########################################################"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-orgTourist.yaml --output="organizations"
    res=$?
    set +x
    if [ $res -ne 0 ]; then
    ...
    echo "##########################################################"
    echo "############ Create OrgToursChateau Identities ######################"
    echo "##########################################################"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-orgToursChateau.yaml --output="organizations"
    res=$?
    set +x
    if [ $res -ne 0 ]; then
      echo "Failed to generate certificates..."
  ...
  echo
  echo "Generate CCP files for OrgTourist and OrgToursChateau"
...
## call the script to join create the channel and join the peers of orgTourist and orgToursChateau
...
function networkDown() {
  # stop org3 containers also in addition to orgTourist and orgToursChateau, in case we were running sample to add org3
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    rm -rf organizations/fabric-ca/orgTourist/msp organizations/fabric-ca/orgTourist/tls-cert.pem organizations/fabric-ca/orgTourist/ca-cert.pem organizations/fabric-ca/orgTourist/IssuerPublicKey organizations/fabric-ca/orgTourist/IssuerRevocationPublicKey organizations/fabric-ca/orgTourist/fabric-ca-server.db
    rm -rf organizations/fabric-ca/orgToursChateau/msp organizations/fabric-ca/orgToursChateau/tls-cert.pem organizations/fabric-ca/orgToursChateau/ca-cert.pem organizations/fabric-ca/orgToursChateau/IssuerPublicKey organizations/fabric-ca/orgToursChateau/IssuerRevocationPublicKey organizations/fabric-ca/orgToursChateau/fabric-ca-server.db
